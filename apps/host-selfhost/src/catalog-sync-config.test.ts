...
test("tool catalog sync TTL uses the SDK default when unset", () => {
  delete process.env[ENV_NAME];
  expect(loadConfig().toolsSyncTtlMs).toBeUndefined();
});

test("tool catalog time-based sync can be disabled", () => {
  process.env[ENV_NAME] = "off";
  expect(loadConfig().toolsSyncTtlMs).toBeNull();
});

test("tool catalog sync TTL accepts positive milliseconds", () => {
  process.env[ENV_NAME] = "3600000";
  expect(loadConfig().toolsSyncTtlMs).toBe(3_600_000);
});

test.each(["0", "-1", "nope"])("tool catalog sync TTL rejects %s", (value) => {
